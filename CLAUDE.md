...
- **OAuth `redirectTo`** : TOUJOURS pointer sur **`/auth/callback?next=<path>`** — jamais sur l'URL courante. Le flow PKCE (`createBrowserClient` de `@supabase/ssr`) revient avec **son propre `?code=<uuid>`** qui écrase `?code=<room>` sur `/join` (bug playtest : UUID affiché dans le champ code). La route callback échange le code server-side puis redirige vers `next` (paths same-origin uniquement). Vérifier que les **Redirect URLs Supabase** couvrent `https://kluup.app/auth/callback`. Défense en profondeur : `/join` valide le param `code` contre `ROOM_CODE_RE` (`^[A-HJ-NP-Z2-9]{6}$`) avant de pré-remplir.
...
- **Type C choice = PAS de timer** (playtest #4 / SC-7) : `ChoiceScreen` n'a **aucun compte à rebours** — la manche n'avance que quand **tous** ont agi (volontaire ou bûcher) ; seul filet : skip hôte. Le timer 30 s forçait la résolution mi-choix → désync perçue (un joueur passait à la phase suivante pendant qu'un autre choisissait encore). Ne pas remettre `round_c_choice` dans les phases à timer.
- **IDEN-02 + même compte Google que l'hôte** (playtest #4) : avant de réutiliser silencieusement une ligne `players` matchée par `user_id`, `/join` vérifie via **présence** que la ligne n'est pas active sur un autre appareil (`isPlayerOnline`). Si active (cas : le joueur se connecte avec le compte Google de l'hôte) → insert d'une **nouvelle ligne avec `user_id: null`** (jamais 2 lignes même `user_id` dans une room — le lookup IDEN-02 utilise `maybeSingle()`). Timeout présence 2,5 s → fallback "offline" pour ne pas casser la reconnexion cross-device légitime.
