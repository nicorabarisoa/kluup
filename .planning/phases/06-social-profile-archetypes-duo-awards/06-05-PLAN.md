...
<objective>
Refactor `ShareCard` and `EndScreen` in `app/room/[code]/game/page.tsx` into the Phase 6 computation hub
...
Phase 6 stays session-only. Do NOT change the `tag_scores: {}` literal in the existing stats-write effect.
...
- No new exported symbols, no new game_state fields, no new tables (all per 06-CONTEXT.md)
</artifacts_produced>
...
<context>
@.planning/phases/06-social-profile-archetypes-duo-awards/06-UI-SPEC.md
...
<task type="auto">
  ...
  <read_first>
    ...
    - .planning/phases/06-social-profile-archetypes-duo-awards/06-PATTERNS.md (EndScreen refactor → votes fetch with single-fire guard, useMemo for archetype + awards, activeCard state, exportCard unchanged)
    - .planning/phases/06-social-profile-archetypes-duo-awards/06-RESEARCH.md (System Architecture Diagram; Pitfall 7 / P-05 single-fetch; open question 3 → explicit votes column list)
  ...
  <action>
    ...
    target_player_id, answer').eq('room_id', roomId)` — use an explicit column list per 06-RESEARCH.md open
...
<task type="auto">
  ...
  <read_first>
    ...
    - .planning/phases/06-social-profile-archetypes-duo-awards/06-UI-SPEC.md (§ Card Architecture — 2-Face Contract; § Flip Interaction Contract; § Capture Container Contract; Face 1 / Face 2 layouts + edge cases D-03)
    - .planning/phases/06-social-profile-archetypes-duo-awards/06-PATTERNS.md (ShareCard refactor → forwardRef prop additions, face swap conditional render not display:none, flip affordance outside capture, off-screen + scaled-preview instances)
    - components/ArchetypeBlock.tsx + components/DuoAwardsBlock.tsx (the blocks to slot into Face 2 / Face 1)
    - .planning/phases/06-social-profile-archetypes-duo-awards/06-RESEARCH.md (Pattern 3; Pitfall 5 / P-07 — never render both faces with display:none)
  </read_first>
  <action>
    ...
    result; fallback archetype still renders its name per 06-UI-SPEC.md edge case), then spacer + pills + footer.
    ...
    wrapper `onClick` toggles `activeCard` (150ms opacity fade, no 3D rotateY — 06-UI-SPEC.md Flip Interaction).
    ...
    No fenced code in this action — copy structure/identifiers from 06-PATTERNS.md ShareCard refactor section.
  </action>
  <verify>
...
<task type="checkpoint:human-verify" gate="blocking-human">
  <what-built>
    The full Phase 6 end-to-end experience: at the end of a game, Face 1 shows the group title + duo awards,
    ...
    (06-VALIDATION.md § Manual-Only Verifications; REQ-DA-04).
  </what-built>
  <how-to-verify>
...
| T-A-SC | Tampering | npm/pip/cargo installs | mitigate | No package installs in this plan (Vitest installed in Plan 01; no `[ASSUMED]`/`[SUS]` packages anywhere in Phase 6). No install checkpoint required here. |
</threat_model>
...
<output>
Create `.planning/phases/06-social-profile-archetypes-duo-awards/06-05-SUMMARY.md` when done
</output>
